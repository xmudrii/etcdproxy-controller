...
const controllerAgentName = "etcdstorage-controller"
...
// Controller is the controller implementation for EtcdStorage resources
...
// NewEtcdStorageController returns a new sample controller
func NewEtcdStorageController(
	kubeclientset kubernetes.Interface,
	sampleclientset clientset.Interface,
	deploymentInformer appsinformers.DeploymentInformer,
	etcdstorageInformer informers.EtcdStorageInformer) *Controller {
...
func (c *Controller) Run(threadiness int, stopCh <-chan struct{}) error {
	...
	if ok := cache.WaitForCacheSync(stopCh, c.deploymentsSynced, c.etcdstoragesSynced); !ok {
...
func (c *Controller) syncHandler(key string) error {
	...
	//deploymentName := etcdstorage.Spec.DeploymentName
	//if deploymentName == "" {
	...
	//}
	//
	//// Get the deployment with the name specified in EtcdStorage.spec
	//deployment, err := c.deploymentsLister.Deployments(etcdstorage.Namespace).Get(deploymentName)
	//// If the resource doesn't exist, we'll create it
	//if errors.IsNotFound(err) {
	//	deployment, err = c.kubeclientset.AppsV1().Deployments(etcdstorage.Namespace).Create(newDeployment(etcdstorage))
	//}
	//
	...
	//}
	//
	//// If this number of the replicas on the EtcdStorage resource is specified, and the
	//// number does not equal the current desired replicas on the Deployment, we
	//// should update the Deployment resource.
	//if etcdstorage.Spec.Replicas != nil && *etcdstorage.Spec.Replicas != *deployment.Spec.Replicas {
	//	glog.V(4).Infof("EtcdStorage %s replicas: %d, deployment replicas: %d", name, *etcdstorage.Spec.Replicas, *deployment.Spec.Replicas)
	//	deployment, err = c.kubeclientset.AppsV1().Deployments(etcdstorage.Namespace).Update(newDeployment(etcdstorage))
	//}
	//
	...
	//err = c.updateEtcdStorageStatus(etcdstorage, deployment)
	//if err != nil {
	//	return err
	//}
...
func (c *Controller) updateEtcdStorageStatus(etcdstorage *samplev1alpha1.EtcdStorage, deployment *appsv1.Deployment) error {
	// NEVER modify objects from the store. It's a read-only, local cache.
	// You can use DeepCopy() to make a deep copy of original object and modify this copy
	// Or create a copy manually for better performance
	etcdstorageCopy := etcdstorage.DeepCopy()
	//etcdstorageCopy.Status.AvailableReplicas = deployment.Status.AvailableReplicas
	// If the CustomResourceSubresources feature gate is not enabled,
	// we must use Update instead of UpdateStatus to update the Status block of the EtcdStorage resource.
	// UpdateStatus will not allow changes to the Spec of the resource,
	// which is ideal for ensuring nothing other than resource status has been updated.
	_, err := c.sampleclientset.EtcdV1alpha1().EtcdStorages().Update(etcdstorageCopy)
	return err
}
...
func (c *Controller) enqueueEtcdStorage(obj interface{}) {
	var key string
	var err error
...
func newDeployment(etcdstorage *samplev1alpha1.EtcdStorage) *appsv1.Deployment {
	labels := map[string]string{
		"app":        "nginx",
		"controller": etcdstorage.Name,
	}
	repls := int32(3)
	return &appsv1.Deployment{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "test",
			Namespace: etcdstorage.Namespace,
